...
- **立项核心指标审计规则**：在开始任何新项目或重大模块重构前，Agent 必须首先挂起开发逻辑，主动向 User 问询并对齐以下核心交付指标：
  - **体积阈值**：可接受的最终打包体积底线（如：是否介意 Chromium 驱动带来 80MB+ 额外开销，是否要求极简 Portable 单文件）？
  - **运行效率底线**：对应用冷启动耗时、密集计算（如 OCR、物理模拟）的最大容忍延迟（毫秒级还是秒级）？
  - **杀软敏感度与权限**：应用是否需要在系统受限的环境下静默运行？
  在 User 明确答复前，Agent 严禁直接敲定技术栈。
...
4. **连续失败根源阻断规则**：当 User 连续两次反馈相同或极度相似的运行异常现象（如“点击后白屏”、“点击按钮无反应”）时，Agent 必须立即判定当前的局部修改逻辑无效。必须立即切入“底层诊断模式”，首先检查系统后台是否有残留的同名僵尸进程并强杀之，或者直接在后端入口处注入 Panic 弹窗与 Debug 校验抛出错误信息，严禁在未搞清死锁或进程锁占用的情况下盲目修补前端组件。
